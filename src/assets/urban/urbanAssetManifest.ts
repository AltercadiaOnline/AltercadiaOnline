...
export type UrbanPropSpec = {
  readonly id: UrbanPropId;
  readonly fileName: string;
  /** Largura em px — múltiplo de 40. */
  readonly widthPx: number;
  /** Altura em px — múltiplo de 40. */
...
export const URBAN_PROP_SPECS: readonly UrbanPropSpec[] = [
  {
    id: 'street_light',
...
const specById = new Map<UrbanPropId, UrbanPropSpec>(
  URBAN_PROP_SPECS.map((spec) => [spec.id, spec]),
);

/** URLs públicas — chave = assetKey usado pelo worldAssetImageLoader. */
export const URBAN_PROP_IMAGE_URLS: Readonly<Record<string, string>> = Object.fromEntries(
  URBAN_PROP_SPECS.map((spec) => [
    spec.id,
    `${URBAN_ASSET_PUBLIC_BASE}/${spec.fileName}`,
  ]),
);

export function getUrbanPropSpec(id: UrbanPropId): UrbanPropSpec {
  const spec = specById.get(id);
  if (!spec) throw new Error(`[urbanAssetManifest] Prop desconhecido: ${id}`);
  return spec;
}

export function resolveUrbanPropImageUrl(assetKey: string): string | null {
  return URBAN_PROP_IMAGE_URLS[assetKey] ?? null;
}

export function listUrbanPropIds(): readonly UrbanPropId[] {
