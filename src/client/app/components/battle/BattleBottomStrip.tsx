...
type BattleBottomStripProps = {
  requestMove: (moveId: string) => void;
  requestItem: (itemId: string) => void;
};

function BattleMovesetConnected({
  onSelectMove,
}: {
 ...
 */
export function BattleBottomStrip({
  requestMove,
  requestItem,
}: BattleBottomStripProps) {
  return (
    <div
      className="battle-hud-bottom-strip pointer-events-auto"
      data-ui-surface="battle-bottom-strip"
    >
      <section
        className="battle-hud-bottom-strip__actions"
        aria-label="Movesets e botões de ação"
      >
      ...
      <section
        className="battle-hud-bottom-strip__comms"
        aria-label="Log e chat da batalha"
      >
        <BattleLogConnected />
        <BattleChatConnected />
      </section>
