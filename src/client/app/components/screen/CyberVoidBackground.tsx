...
/**
 * Fundo Cyber-Void.
 * Normal: canvas com grade + partículas subindo (visual original).
 * Leve: grade CSS, sem rAF.
 */
function CyberVoidCanvas() {
  ...
  useEffect(() => {
    ...
    const drawParticles = (w: number, h: number, dt: number): void => {
      ...
      for (const particle of particles) {
        if (!reducedMotion) {
          particle.y -= particle.speed * particle.z * dt * 0.0236;
          if (particle.y < -0.04) {
            particle.y = 1.04;
        ...
        const alpha = 0.18 + particle.z * 0.42;
        ctx.fillStyle = `rgba(220, 244, 255, ${alpha})`;
        if (particle.glyph) {
          ctx.fillText(particle.glyph, px, py);
        } else {
          ctx.beginPath();
          ctx.arc(px, py, 1.1 + particle.z * 1.8, 0, Math.PI * 2);
          ctx.fill();
        }
      }
    };
...
export function CyberVoidBackground() {
  const lite =
    useSyncExternalStore(subscribePerformanceMode, getPerformanceMode, getPerformanceMode) === 'lite';
  return lite ? <LiteVoidFallback /> : <CyberVoidCanvas />;
}
