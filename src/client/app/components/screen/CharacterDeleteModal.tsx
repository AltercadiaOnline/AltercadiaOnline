...
export function CharacterDeleteModal({ open, characterId, characterName, onClose, }) {
    ...
    return (<div className="char-create-overlay" aria-hidden="false" role="dialog" aria-modal="true" aria-labelledby="char-delete-title" onClick={(event) => {
            if (event.target === event.currentTarget && !busy) {
                onClose();
            }
        }}>
      <div className="char-create-box vortex-panel auth-form char-delete-box">
        ...
        <div className="auth-actions">
          <button type="button" className="char-delete-confirm" disabled={busy} aria-busy={busy} onClick={handleConfirm}>
        ...
        {statusMessage && (<p className={`auth-status ${statusIsError ? 'is-error' : ''}`} aria-live="polite">
            {statusMessage}
          </p>)}
      </div>
    </div>);
