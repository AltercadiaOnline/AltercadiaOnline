...
export function HybridHudFoundation() {
    ...
    const [uiSnapshot, setUiSnapshot] = useState(() => readUiSnapshot());
    const [renderSnapshot, setRenderSnapshot] = useState(() => readRenderSnapshot());
    useEffect(() => getGameUiBridge().subscribe(setUiSnapshot), []);
    useEffect(() => getRenderLayerBridge().subscribe(setRenderSnapshot), []);
    return (<div className="pointer-events-none absolute right-3 top-3 z-[922]">
