...
import { getPvpQueueStore } from '../../../panels/pvpQueueStore.js';
...
import { PVP_RANKED_ACCEPT_COUNTDOWN_MS, PVP_RANKED_MODE, PVP_RANKED_STATION_ID, PVP_RANKED_STATION_LABEL, } from '../../../../../shared/combat/pvp/pvpRankedQueueConfig.js';
...
function resolveStation(context) {
...
function resolveLocalPvpPlayerId() {
    try {
...
function usePvpQueueSnapshot() {
    const store = getPvpQueueStore();
    return useSyncExternalStore((onStoreChange) => store.subscribe(onStoreChange), () => store.getSnapshot(), () => store.getSnapshot());
}
function PvpFighterCard({ slot, side, }) {
    const previewUrl = slot
        ? resolvePlayerSkinBundleSouthPreviewUrl(slot.skinBundleId)
        : null;
    return (<div className={`pvp-queue__fighter pvp-queue__fighter--${side}${slot ? ' is-filled' : ''}${slot?.ready ? ' is-ready' : ''}`}>
      <div className="pvp-queue__fighter-frame">
        {previewUrl ? (<img className="pvp-queue__fighter-skin" src={previewUrl} alt="" draggable={false}/>) : (<div className="pvp-queue__fighter-empty" aria-hidden>
            ?
          </div>)}
      </div>
...
export function WorldPvpQueuePanel({ context, zIndex, focused, }) {
    ...
    const countdownLabel = snapshot.countdownSecondsRemaining !== null
    ...
    useEffect(() => {
        ...
        store.ensureLocalPresent(localPlayerId, profile.displayName || 'Você', getActivePlayerSkinBundleId());
        return () => {
            // Strict Mode remount: painel ainda aberto → não cancela a sessão.
            queueMicrotask(() => {
                if (isWorldPanelOpen('pvpQueue'))
                    return;
    ...
    useEffect(() => {
        return store.onRankedMatchStart((match) => {
            postGameChatMessage(`PvP rankeado ${PVP_RANKED_MODE}: ${match.slots[0]?.displayName ?? '?'} vs ${match.slots[1]?.displayName ?? '?'} — entrando na batalha…`);
    ...
    return (<MovablePanelFrame windowId="pvpQueue" title={station.label} zIndex={zIndex} focused={focused} panelClassName="world-panel--pvp-queue ui-panel--pvp-queue" panelStyle={{ width: 'min(520px, 96vw)' }} onFocus={() => tryFocusReactWorldPanel('pvpQueue')} onClose={() => {
            store.leaveLocal(localPlayerId);
            tryCloseReactWorldPanel('pvpQueue');
        }}>
      <div className="pvp-queue">
        ...
        {countdownActive ? (<div className="pvp-queue__countdown pvp-queue__countdown--hero" aria-live="polite">
        ...
        <div className="pvp-queue__actions">
          {inQueue && !localSlot?.ready && !countdownActive ? (<button type="button" className="pvp-queue__btn pvp-queue__btn--primary pvp-queue__btn--enter" disabled={!snapshot.slots[0] || !snapshot.slots[1]} onClick={() => store.requestEnterRanked(localPlayerId)}>
              Entrar na batalha rankeada
            </button>) : null}

          {inQueue && localSlot?.ready && snapshot.phase === 'waiting' ? (<button type="button" className="pvp-queue__btn" onClick={() => store.cancelEnterRanked(localPlayerId)}>
              Cancelar aceite
            </button>) : null}

          {inQueue && !countdownActive ? (<button type="button" className="pvp-queue__btn" onClick={() => {
                store.leaveLocal(localPlayerId);
                tryCloseReactWorldPanel('pvpQueue');
            }}>
              Sair
          ...
          {!snapshot.slots[1] && !countdownActive ? (<button type="button" className="pvp-queue__btn pvp-queue__btn--ghost" onClick={() => store.fillOpponentStub('Oponente')}>
              [Dev] Simular oponente
            </button>) : snapshot.slots[1] && !snapshot.slots[1].isLocal && !snapshot.slots[1].ready && !countdownActive ? (<button type="button" className="pvp-queue__btn pvp-queue__btn--ghost" onClick={() => store.setOpponentReady(true)}>
              [Dev] Oponente entra
            </button>) : null}
        </div>
      </div>
