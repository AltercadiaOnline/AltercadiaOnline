...
export function WorldInventoryPanel({ zIndex, focused }: WorldInventoryPanelProps) {
  const { inventory, gold, vendorOpen } = useInventoryPanelState();
  ...
  // Só o botão da ação fica busy — slots espelham inventário, sem ⟳ global.
  const actionsBusy = deletePending || equipPending;
