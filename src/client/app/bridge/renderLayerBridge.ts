...
export type RenderEngine = 'phaser';
...
class RenderLayerBridge {
  private renderEngine: RenderEngine = 'phaser';
...
/** Modo Phaser ativo e cena pronta — publicação segura de frames de exploração. */
...
/** Player/NPC/pet montados no Phaser. */
...
export function resolveRenderHostElement(): HTMLElement {
  ...
  if (phaserHost instanceof HTMLElement) {
    return phaserHost;
  }
  const renderHost = document.getElementById('game-render-host');
  ...
  throw new Error('[render-layer] Host de renderização Phaser indisponível');
}
