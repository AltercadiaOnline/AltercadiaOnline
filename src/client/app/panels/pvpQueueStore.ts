 ...
 */
import { PVP_RANKED_ACCEPT_COUNTDOWN_MS, PVP_RANKED_QUEUE_SLOT_COUNT, PVP_RANKED_STATION_ID, PVP_RANKED_STATION_LABEL, } from '../../../shared/combat/pvp/pvpRankedQueueConfig.js';
const EMPTY_SLOTS = [null, null];
function emptySnapshot(objectId = PVP_RANKED_STATION_ID, label = PVP_RANKED_STATION_LABEL) {
    return {
        objectId,
        label,
        phase: 'idle',
...
class PvpQueueStore {
     ...
     */
    openStation(objectId, label) {
        if (this.snapshot.objectId === objectId
    ...
    ensureLocalPresent(playerId, displayName, skinBundleId) {
    ...
    leaveLocal(playerId) {
        ...
        if (!wasPresent)
            return;
        const hadLinkedSession = this.snapshot.phase === 'countdown'
     ...
     */
    fillOpponentStub(displayName = 'Oponente', skinBundleId = 'player_male_2') {
        if (this.snapshot.slots[1])
            return;
        if (this.snapshot.phase === 'countdown' || this.snapshot.phase === 'starting')
            return;
        const slots = [
    ...
    setOpponentReady(ready = true) {
        const opponent = this.snapshot.slots[1];
        if (!opponent || opponent.isLocal)
            return;
        const slots = [
            this.snapshot.slots[0],
            { ...opponent, ready },
...
export function getPvpQueueStore() {
    const g = globalThis;
