...
export function isPhaserHybridDevRequested() {
    if (typeof window === 'undefined')
        return false;
    const params = new URLSearchParams(window.location.search);
    if (params.get(PHASER_HYBRID_QUERY) === '1')
        return true;
    try {
...
export function maybeEnablePhaserFromDevFlags() {
 ...
 */
export function initPhaserReadyLayer() {
...
export function teardownPhaserReadyLayer() {
...
export function enablePhaserRenderMode() {
...
export function disablePhaserRenderMode() {
    getRenderLayerBridge().setRenderEngine('phaser');
}
