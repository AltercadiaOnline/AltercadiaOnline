...
export class BattleMenu {
    ...
    handleContainerClick = (event) => {
        if (!this.options.enabled || !this.onMoveSelected)
            return;
        const target = event.target;
        if (!(target instanceof Element))
            return;
        const button = target.closest('.battle-menu-btn:not(.is-empty):not(:disabled)');
    ...
    render(options) {
        ...
        const slots = [...options.moves.slice(0, ACTIVE_MOVESET_SLOT_COUNT)];
        while (slots.length < ACTIVE_MOVESET_SLOT_COUNT)
            slots.push(null);
        for (const move of slots) {
            ...
            if (outOfPp)
                btn.classList.add('is-no-pp');
            else if (lowPp)
                btn.classList.add('is-low-pp');
    ...
    bindMoveTooltip(element, moveId) {
        const onEnter = () => {
    ...
    clearingTooltips = false;
    clearTooltipListeners() {
        if (this.clearingTooltips)
            return;
