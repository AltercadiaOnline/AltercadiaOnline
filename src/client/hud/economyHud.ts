...
export function formatDollarVolt(amount) {
    return formatVolts(amount);
}
export function applyEconomyEventToHud(event) {
    ...
    if (event.type === EconomyEventType.UpdateBankSuccess) {
        const slots = stacksToInventorySlots(event.payload.inventory.map((row) => ({
            itemId: row.itemId,
        ...
        const bankView = buildBankStorageView(event.payload.bankItemStacks, event.payload.bankCurrencies);
...
export function isEconomyEvent(value) {
    if (!value || typeof value !== 'object')
        return false;
    const record = value;
    if (typeof record.type !== 'string')
        return false;
    return Object.values(EconomyEventType).includes(record.type);
}
/** Emite eventos de UI a partir do espelho local (exploração offline). */
export function publishLocalWalletToHud() {
