// @ts-nocheck
import { GAME_RENDER_HEIGHT, GAME_RENDER_WIDTH, } from '../layout/gameLayout.js';
import { configureFixedGameCanvas, disableCanvasImageSmoothing } from '../layout/gamePixelScale.js';
import { drawDepthSortedInPlace } from '../../shared/world/worldDepthSort.js';
...
export class GameRenderer {
    ...
    pinNativeCanvasResolution(canvas) {
    ...
    clearBuffer(ctx, state) {
        ctx.fillStyle = state.clearColor;
        ctx.fillRect(0, 0, snapToPixel(state.viewport.width), snapToPixel(state.viewport.height));
    }
    renderWorldLayers(ctx, state) {
        ctx.save();
    ...
    renderScreenSpaceLayer(ctx, state) {
