...
export function preloadTiledMapCanvasAssets(mapId) {
 ...
 */
export function renderTiledMapGroundFromExport(ctx, mapId, camera) {
    const descriptor = resolveTiledMapDescriptor(mapId);
    ...
    const tilesets = parseTilesets(mapId).filter((tileset) => tileset.tilewidth === mapTileWidth && tileset.tileheight === mapTileHeight);
    ...
    const endX = Math.min(mapWidthTiles, Math.ceil((camera.x + camera.visibleWorldWidth) / mapTileWidth) + pad);
    const endY = Math.min(mapHeightTiles, Math.ceil((camera.y + camera.visibleWorldHeight) / mapTileHeight) + pad);
    ...
    for (const layer of layers) {
        ...
        if (entry.type !== 'tilelayer' || !Array.isArray(entry.data))
            continue;
        if (typeof entry.name !== 'string' || !isTiledVisualTileLayer(entry.name))
            continue;
        ...
        for (let y = startY; y < endY; y += 1) {
            for (let x = startX; x < endX; x += 1) {
                ...
                ctx.drawImage(image, sx, sy, tileset.tilewidth, tileset.tileheight, dx, dy, mapTileWidth, mapTileHeight);
                drewAny = true;
            }
        }
    }
