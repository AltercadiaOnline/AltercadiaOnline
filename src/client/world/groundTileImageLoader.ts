// @ts-nocheck
import { GROUND_TILE_IMAGE_URLS, PLACEHOLDER_TYPE_TO_GROUND_TILE, } from '../../assets/terrain/groundTileManifest.js';
...
export function resolveGroundTileId(type) {
    return PLACEHOLDER_TYPE_TO_GROUND_TILE[type] ?? null;
}
export function preloadGroundTile(tileId) {
...
export function getCachedGroundTile(tileId) {
    if (!cache.has(tileId)) {
...
export function drawGroundTileImage(ctx, type, x, y, size) {
    ...
    if (!image?.complete || image.naturalWidth <= 0)
        return false;
    const plan = planCanvasAssetDraw(image.naturalWidth, image.naturalHeight, size, size, `${tileId}.png`);
...
export function resetGroundTileImageCache() {
    cache.clear();
    pending.clear();
}
