 ...
 */
import { CREATURE_ENCOUNTER_FLEE_COOLDOWN_MS, CREATURE_ENCOUNTER_IDLE_TIMEOUT_MS, CREATURE_RESPAWN_MS, isPlayerInMonsterEncounterRange, resolveCreatureWanderProfile, } from '../../shared/world/creatureWanderConfig.js';
import { isMapId } from '../../shared/world/mapRegistry.js';
import { tileCenterToWorldPixel } from '../../shared/world/portals.js';
import { getActiveMonstersForMap, getWorldMonsterEntry, getWorldMonsterEntryRaw, restoreWorldMonsterAfterRespawn, stashWorldMonsterForRespawn, } from '../../shared/world/worldMonsterInstances.js';
...
let exploringProvider = null;
let tickTimer = null;
let pending = null;
let forceBattleNext = false;
const fleeCooldownUntil = new Map();
const respawnAtById = new Map();
function clearPendingUi(reason) {
    if (!pending)
        return;
...
function abandonWithoutDecision(reason) {
    if (!pending)
        return;
    forceBattleNext = true;
    clearPendingUi(reason);
}
function findCandidates(mapId, playerWorldX, playerWorldY) {
    const tileSize = resolveMapTileSize(mapId);
    return getActiveMonstersForMap(mapId).filter((monster) => isPlayerInMonsterEncounterRange(playerWorldX, playerWorldY, monster, tileSize));
}
function emitOffer(monster, nowMs) {
    pending = {
...
async function startLocalBattle(monsterInstanceId) {
 ...
 */
export function tryAcceptLocalPveEncounter(monsterInstanceId) {
...
function beginForcedBattle(monster) {
    forceBattleNext = false;
    pending = null;
...
function tickLocalEncounters(nowMs) {
    if (getGameMode() !== 'local')
        return;
    if (!exploringProvider?.())
        return;
    // Respawn
    for (const [id, entry] of [...respawnAtById]) {
        ...
        if (isMapId(entry.template.mapId)) {
            const snapshots = getActiveMonstersForMap(entry.template.mapId).map(monsterEntryToCreatureSnapshot);
            applyServerWorldCreatureSnapshots(entry.template.mapId, snapshots);
        }
    }
    const pose = poseProvider?.();
    if (!pose || !isMapId(pose.mapId))
        return;
    ...
    if (pending) {
        ...
        if (!isPlayerInMonsterEncounterRange(pose.worldX, pose.worldY, monster, tileSize)) {
            abandonWithoutDecision('moved_away');
        }
        return;
    }
...
export function stopLocalPveEncounterRuntime() {
    if (tickTimer) {
...
export function resetLocalPveEncounterRuntime() {
    stopLocalPveEncounterRuntime();
    forceBattleNext = false;
...
export function dispatchLocalPveEncounter(type, payload) {
    if (getGameMode() !== 'local')
        return;
    ...
    if (type === 'pve-encounter-request') {
        ...
        if (pending && pending.monsterInstanceId !== monsterId)
            return;
    ...
    pending = null;
    const fled = Math.random() < resolveCreatureWanderProfile(creatureId).fleeSuccessChance;
    if (fled) {
...
export function scheduleLocalMonsterRespawn(monsterId, nowMs = Date.now()) {
