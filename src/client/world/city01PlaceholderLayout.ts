...
import { CITY_01_MAP_TILES, CITY_01_COMMERCE_ZONE, CITY_01_RESIDENTIAL_ZONE, CITY_01_PLAZA_MAX, CITY_01_PLAZA_MIN, CITY_01_ARENA_CORE, CITY_01_ARENA_SPECTATOR_RING, CITY_01_ARENA_VISUAL, CITY_01_ROAD_NORTH_Y, CITY_01_ROAD_SOUTH_Y, CITY_01_ROAD_X_MAX, CITY_01_ROAD_X_MIN, CITY_01_ROAD_Y_MAX, CITY_01_ROAD_Y_MIN, CITY_01_STRUCTURE_DEFS, isCity01RoadNetworkTile, } from '../../shared/world/maps/city01LayoutConstants.js';
...
import { CITY_01_TEST_PACK_DECORATIVE_PROPS, CITY_01_TEST_PACK_WALL_PROPS, } from '../../game/generated/city01TestPackWiring.js';
...
function buildStructureEntities() {
    return CITY_01_STRUCTURE_DEFS.map((structure) => {
        ...
        const isInteractive = structure.id === 'food_block' || structure.id === 'market_block';
        return {
            assetKey,
...
function buildWorldObjectEntities() {
...
function buildTestPackPropEntities() {
...
function buildUrbanPropEntities() {
...
export function buildCity01PlaceholderScene(mapTiles = CITY_01_MAP_TILES) {
...
export function entityAtTile(scene, tileX, tileY) {
    for (const entity of [...scene.entities, ...scene.npcSpots]) {
        if (tileX >= entity.tileX &&
...
export function sceneTileToWorld(tileX, tileY, tileSize) {
