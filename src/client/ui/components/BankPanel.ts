...
import { BANK_HUD_GRID_COLUMNS, BANK_HUD_GRID_ROWS, BANK_ITEM_SLOT_CAPACITY, BankCurrencyType, } from '../../../shared/bank/bankConstants.js';
import { normalizeBankCurrencyAmount } from '../../../shared/bank/bankCurrencyRules.js';
import { clampBankVaultPageIndex, sliceBankVaultPageSlots, } from '../../../shared/bank/bankVaultPagination.js';
...
export class BankPanel extends BaseUIComponent {
    ...
    unsubInventory = null;
    unsubBank = null;
    unsubBankSuccess = null;
    unsubBankFail = null;
    unsubBankBalance = null;
    constructor() {
    ...
    bindDataStoreSubscriptions() {
        ...
        this.unsubBank = store.subscribe('bankStorage', (bank) => {
            ...
            this.vaultCurrentPage = clampBankVaultPageIndex(this.vaultCurrentPage, bank.itemCapacity);
            if (this.isOpen()) {
    ...
    teardownDataStoreSubscriptions() {
        this.unsubWallet?.();
        this.unsubInventory?.();
        this.unsubBank?.();
    ...
    isBankTransactionInFlight() {
        return this.pendingFlow !== null || getPendingIntentRegistry().hasPendingBankTransaction();
    }
    patchBankCurrencyBalances() {
        if (!this.root)
            return;
        const voltsEl = this.root.querySelector('[data-bank-volts]');
        const alterEl = this.root.querySelector('[data-bank-alter]');
        if (voltsEl)
            voltsEl.textContent = this.bankStorage.voltsFormatted;
        if (alterEl)
            alterEl.textContent = this.bankStorage.alterFormatted;
    }
    setBankControlsDisabled(disabled) {
        if (!this.root)
            return;
        const flag = disabled ? 'true' : 'false';
        for (const el of this.root.querySelectorAll('[data-action="confirm-deposit"], [data-action="confirm-withdraw"], [data-action="clear-staged"], '
            + '[data-action="deposit-volts"], [data-action="withdraw-volts"], '
            + '[data-action="deposit-alter"], [data-action="withdraw-alter"]')) {
            el.disabled = disabled;
        }
        for (const el of this.root.querySelectorAll('[data-bank-select-slot]')) {
            if (disabled)
                el.setAttribute('aria-disabled', flag);
            else
                el.removeAttribute('aria-disabled');
        }
        const qtyInput = this.root.querySelector('[data-qty-input]');
        if (qtyInput)
            qtyInput.disabled = disabled || !this.stagedTransfer;
    }
    createTemplate() {
    ...
    renderItemsTab() {
        ...
        const allVaultSlots = stacksToInventorySlotsWithStacking(this.bankStorage.itemStacks, BANK_ITEM_SLOT_CAPACITY);
        const vaultPage = sliceBankVaultPageSlots(allVaultSlots, this.vaultCurrentPage, this.bankStorage.itemCapacity);
    ...
    isSlotStaged(source, slotIndex) {
        return (this.stagedTransfer?.source === source
    ...
    renderVaultPagination(currentPage, pageCount) {
        if (pageCount <= 1) {
    ...
    setVaultPage(pageIndex) {
        this.vaultCurrentPage = clampBankVaultPageIndex(pageIndex, this.bankStorage.itemCapacity);
        if (this.stagedTransfer?.source === 'bank') {
            ...
            if (this.stagedTransfer.slotIndex < pageStart
    ...
    resolveFlowClass() {
        if (this.pendingFlow === 'to-vault')
            return 'is-flow-to-vault';
        if (this.pendingFlow === 'to-inventory')
            return 'is-flow-to-inventory';
        return '';
    }
    resolveStagedTransfer() {
        if (!this.stagedTransfer)
            return null;
        if (this.stagedTransfer.source === 'inventory') {
            const slot = this.inventory.slots[this.stagedTransfer.slotIndex];
            if (!slot?.itemId || slot.quantity <= 0)
                return null;
            if (slot.itemId !== this.stagedTransfer.itemId)
                return null;
            if (this.isCurrencyItem(slot.itemId))
                return null;
        ...
        const vaultSlots = stacksToInventorySlotsWithStacking(this.bankStorage.itemStacks, BANK_ITEM_SLOT_CAPACITY);
    ...
    stageItemFromSlot(source, slotIndex) {
        if (source === 'inventory') {
            ...
            if (this.stagedTransfer?.source === source
                && this.stagedTransfer.slotIndex === slotIndex) {
                this.stagedTransfer = null;
                this.itemQuantity = 1;
                this.render();
    ...
    afterRender() {
        ...
        if (this.activeTab !== 'items')
            return;
        const cleanups = [];
        for (const slot of this.root.querySelectorAll('[data-item-id]')) {
            const onEnter = (event) => {
    ...
    dispatchItem(type, itemId, flow) {
        if (!itemId || this.isBankTransactionInFlight())
            return;
    ...
    dispatchCurrency(type, currency) {
        if (this.isBankTransactionInFlight()) {
            alertSystem('Aguarde a conclusão da transação bancária anterior.');
            return;
        }
        const input = this.root?.querySelector(`[data-currency-input="${currency}"]`);
