...
import { buildConsumableShopEffectLines, resolveConsumableShopSubtitle, } from '../vendor/consumableShopDisplay.js';
import { resolveLabQuantityPresets, resolveMaxLabPurchaseQuantity, } from '../vendor/labPurchaseHelpers.js';
...
export class LaboratoryShopPanel extends BaseUIComponent {
    ...
    open() {
        if (openReactMovablePanel(this, 'laboratoryShop'))
            return;
        super.open();
    }
    close() {
        if (closeReactMovablePanel(this, 'laboratoryShop'))
            return;
        super.close();
    }
    focus() {
        if (focusReactMovablePanel(this, 'laboratoryShop'))
            return;
        super.focus();
    }
    getRootElement() {
    ...
    createTemplate() {
        const listings = filterLabListingsByTab(getNpcVendorListings(this.vendor.vendorId), this.activeTab);
        const selectedListing = this.selectedItemId
    ...
    renderPurchasePanel(listing) {
        ...
        const maxQty = resolveMaxLabPurchaseQuantity(listing.itemId, this.inventory, this.wallet.dollarVolt, buyUnit);
    ...
    updateWalletLabel() {
        const el = this.query('[data-lab-wallet]');
        if (el)
    ...
    updatePurchasePanel() {
        ...
        const detail = this.query('.laboratory-shop__detail');
        if (detail) {
            detail.innerHTML = this.renderPurchasePanel(listing);
        }
    }
    clampPurchaseQuantity(listing) {
        const buyUnit = resolveEffectiveNpcBuyUnitPrice(listing.itemId, listing) ?? 0;
        const maxQty = resolveMaxLabPurchaseQuantity(listing.itemId, this.inventory, this.wallet.dollarVolt, buyUnit);
    ...
    bindEvents() {
        ...
        this.root?.addEventListener('input', (event) => {
            const target = event.target;
            if (!(target instanceof HTMLInputElement) || !this.selectedItemId)
                return;
            const listing = findNpcVendorListing(this.vendor.vendorId, this.selectedItemId);
            if (!listing)
                return;
            if (target.matches('[data-lab-qty], [data-lab-qty-slider]')) {
                ...
                this.clampPurchaseQuantity(listing);
                const qtyInput = this.query('[data-lab-qty]');
                const qtySlider = this.query('[data-lab-qty-slider]');
                if (qtyInput)
                    qtyInput.value = String(this.purchaseQuantity);
                if (qtySlider)
                    qtySlider.value = String(this.purchaseQuantity);
                const quote = resolveNpcPurchaseQuote(listing, this.purchaseQuantity);
                const totalEl = this.query('[data-lab-buy-total]');
                if (totalEl && quote) {
                    totalEl.textContent = formatVoltsShort(quote.totalVolts);
                }
                const buyBtn = this.query('[data-action="confirm-purchase"]');
                if (buyBtn) {
