...
import { buildDefaultMarketOfferFormState, clampMarketOfferQuantity, } from '../market/marketOfferFormHelpers.js';
import { getMarketplaceOrderBookSnapshot, resolveOwnMarketOfferRef, subscribeMarketplaceOrderBook, } from '../market/marketplaceOrderBookClient.js';
...
import { bindDelegatedItemIconFallback, renderItemIconHtml, } from '../items/itemIconDisplay.js';
...
export class MarketPanel extends BaseUIComponent {
    ...
    browseCategory = 'all';
    searchQuery = '';
    offerForm = buildDefaultMarketOfferFormState(this.inventory);
    unsubInventory = null;
    unsubWallet = null;
    unsubOrderBook = null;
    constructor() {
    ...
    onOpen() {
        ...
        this.offerForm = buildDefaultMarketOfferFormState(this.inventory, this.offerForm.selectedItemId);
        this.ensureSelectedBrowseItem();
        this.unsubInventory = this.dataStore.subscribe('inventory', (snapshot) => {
            this.inventory = snapshot;
            this.offerForm = {
                ...this.offerForm,
                quantity: clampMarketOfferQuantity(this.offerForm.offerSide, this.offerForm.selectedItemId, this.offerForm.quantity, snapshot),
            };
            if (this.isOpen())
                this.render();
        });
    ...
    bindEvents() {
        ...
        this.root?.addEventListener('input', (event) => {
            ...
            if (target.matches('[data-market-offer-qty]')) {
                this.offerForm = {
                    ...this.offerForm,
                    quantity: clampMarketOfferQuantity(this.offerForm.offerSide, this.offerForm.selectedItemId, Number(target.value) || 1, this.inventory),
                };
                this.patchFooterTotals();
                return;
            }
    ...
    renderPreservingSidebarScroll() {
        const list = this.query('.market-terminal__item-list');
        const scrollTop = list?.scrollTop ?? 0;
        this.render();
        const nextList = this.query('.market-terminal__item-list');
        if (nextList)
            nextList.scrollTop = scrollTop;
    }
    selectBrowseItem(itemId) {
        if (this.offerForm.selectedItemId === itemId)
            return;
    ...
    patchItemSelection(itemId) {
        ...
        for (const button of list.querySelectorAll('[data-market-item]')) {
    ...
    patchCenterForItem(itemId) {
        const center = this.query('.market-terminal__center');
        if (!center) {
            this.render();
            return;
        }
    ...
    patchOfferSideToggle() {
        const sellActive = this.offerForm.offerSide === 'sell';
        for (const button of this.root?.querySelectorAll('[data-market-side]') ?? []) {
            const active = button.dataset.marketSide === this.offerForm.offerSide;
            button.classList.toggle('is-active', active);
            button.setAttribute('aria-pressed', active ? 'true' : 'false');
        }
        const qtyInput = this.query('[data-market-offer-qty]');
    ...
    publishOffer() {
        ...
        if (!form.selectedItemId)
            return;
        const qty = clampMarketOfferQuantity(form.offerSide, form.selectedItemId, form.quantity, this.inventory);
    ...
    patchWalletLabel() {
        const el = this.query('[data-market-wallet]');
        if (el)
    ...
    patchFooterTotals() {
        const itemId = this.offerForm.selectedItemId;
        const qty = itemId
            ? clampMarketOfferQuantity(this.offerForm.offerSide, itemId, this.offerForm.quantity, this.inventory)
            : 1;
        const total = qty * Math.max(1, this.offerForm.unitPriceVolts);
        const totalEl = this.query('[data-market-offer-total] strong');
        if (totalEl)
            totalEl.textContent = formatMarketVolts(total);
        const submit = this.query('[data-action="publish-offer"]');
        if (!submit || !itemId)
            return;
        const sellRows = listMarketSellInventoryRows(this.inventory);
