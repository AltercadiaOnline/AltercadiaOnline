...
import { renderPetLoveActivateControl, renderPetLoveRosterDetail, renderPetLoveRosterHud, } from '../pet/petLoveRosterView.js';
...
export class PetLovePanel extends BaseUIComponent {
    roster = getPlayerPetStore().getRoster();
    rationCharges = getPlayerPetStore().getRationCharges();
    feedInlineError = null;
    unsubRoster = null;
    unsubRationCharges = null;
    affectionCooldownTimer = null;
    rationCooldownTimer = null;
    constructor() {
        super({
            id: 'petLove',
    ...
    patchRosterSelection(roster) {
        if (!this.root)
            return;
        for (const tab of this.root.querySelectorAll('[data-pet-slot]')) {
        ...
        const feedAvailability = getPlayerPetStore().getPetRationFeedAvailability();
        if (detail) {
            detail.innerHTML = renderPetLoveRosterDetail(roster, feedAvailability, this.rationCharges, this.feedInlineError);
        }
