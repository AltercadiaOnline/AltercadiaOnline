...
import { findMarcoNodeView, renderMarcoBranchConfirmOverlay, renderMarcoGrid, renderMarcoGridMembranes, renderMarcoGridNodes, resolveMarcoProgressPatch, } from '../marcos/renderMilestoneTree.js';
...
export class MilestoneSkillsPanel extends BaseUIComponent {
    ...
    bindEvents() {
        ...
        this.root?.addEventListener('mouseleave', (event) => {
            if (!(event.target instanceof HTMLElement))
                return;
            const grid = this.query('[data-marcos-grid]');
    ...
    refreshTreeVisuals() {
        ...
        if (membraneHost)
            membraneHost.outerHTML = renderMarcoGridMembranes(model);
        const nodesHost = this.query('[data-marcos-nodes]');
        if (nodesHost)
            nodesHost.innerHTML = renderMarcoGridNodes(model);
    }
    refreshTree() {
        if (!this.isOpen())
            return;
    ...
    buildMarcosStructuralKey(state) {
    ...
    syncMarcoProgressBars() {
        const model = this.buildRenderModel();
        for (const nodeView of model.nodes) {
            if (nodeView.status !== 'active')
                continue;
            const nodeBtn = this.query(`[data-marco-node="${nodeView.def.id}"]`);
            if (!nodeBtn)
                continue;
            const patch = resolveMarcoProgressPatch(nodeView);
            const xpEl = nodeBtn.querySelector('.marcos-node__xp');
            if (xpEl)
                xpEl.textContent = patch.xpLabel;
            const progressEl = nodeBtn.querySelector(`[data-marco-progress="${nodeView.def.id}"]`);
            const fillEl = progressEl?.querySelector('.marcos-node__progress-fill');
            if (patch.showBar) {
                ...
                patchProgressionTooltipAttrs(progressEl, buildMarcoNodeProgressionTooltip(nodeView));
            }
            else if (progressEl) {
                progressEl.remove();
            }
