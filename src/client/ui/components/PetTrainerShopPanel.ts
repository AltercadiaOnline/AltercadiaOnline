// @ts-nocheck
import { getPetDefinition, PET_KIND_ORDER, TREINADOR_ZENO_NPC, } from '../../../shared/pet/petCatalog.js';
...
export class PetTrainerShopPanel extends BaseUIComponent {
    ...
    petName = '';
    selectedColor = null;
    selectedGender = getDefaultPetGenderId();
    unsubWallet = null;
    constructor() {
        super({
            id: 'petTrainerShop',
    ...
    open() {
        if (openReactMovablePanel(this, 'petTrainerShop'))
            return;
        super.open();
    }
    close() {
        if (closeReactMovablePanel(this, 'petTrainerShop'))
            return;
        super.close();
    }
    focus() {
        if (focusReactMovablePanel(this, 'petTrainerShop'))
            return;
        super.focus();
    }
    getRootElement() {
    ...
    createTemplate() {
        ...
        const cards = PET_KIND_ORDER.map((kindId) => this.renderCompanionCard(kindId, roster.pets.some((pet) => pet.kindId === kindId))).join('');
    ...
    renderCompanionCard(kindId, owned) {
        const def = getPetDefinition(kindId);
    ...
    bindEvents() {
        this.root?.addEventListener('click', (event) => {
            ...
            if (colorBtn?.dataset.petColor) {
                this.selectedColor = colorBtn.dataset.petColor;
                this.render();
                return;
            }
            const genderBtn = target.closest('[data-pet-gender]');
            if (genderBtn?.dataset.petGender) {
                this.selectedGender = genderBtn.dataset.petGender;
                this.render();
                return;
            }
            ...
            if (card?.dataset.petKind) {
                if (card.classList.contains('pet-trainer-card--owned'))
                    return;
                this.selectKind(card.dataset.petKind);
            }
        });
    ...
    paintPreviews() {
        ...
        import('../../entities/pet/petRenderer.js').then(({ renderPetShopPreview }) => {
            if (this.customizeOpen && this.selectedKind) {
                const canvas = this.root?.querySelector('[data-pet-custom-preview]');
                const ctx = canvas?.getContext('2d');
                if (canvas && ctx) {
                    ctx.clearRect(0, 0, canvas.width, canvas.height);
                    const colorId = this.selectedColor ?? getDefaultPetColorId(this.selectedKind);
                    renderPetShopPreview(ctx, this.selectedKind, 8, 8, canvas.width - 16, colorId, this.selectedGender);
                }
                return;
            }
            for (const kindId of PET_KIND_ORDER) {
                const canvas = this.root?.querySelector(`[data-pet-preview="${kindId}"]`);
                const ctx = canvas?.getContext('2d');
                if (!canvas || !ctx)
                    continue;
                ctx.clearRect(0, 0, canvas.width, canvas.height);
    ...
    openCustomizeStep() {
        if (!this.selectedKind || this.isKindOwned(this.selectedKind))
            return;
    ...
    updateWalletLabel() {
        const el = this.query('[data-wallet-volts]');
        if (el)
    ...
    async purchaseSelected() {
        if (!this.selectedKind)
            return;
        const colorId = this.selectedColor ?? getDefaultPetColorId(this.selectedKind);
        const nameInput = this.root?.querySelector('[data-pet-name-input]');
    ...
    onPurchaseSettled(petName, options = {}) {
        if (options.notify) {
            alertSystem(`${petName} adotado com sucesso!`);
        }
        this.requestClose();
    }
    awaitPurchaseIntent(intentId, kindId, petName) {
        const registry = getPendingIntentRegistry();
        const finalize = () => {
            if (registry.isIntentPending(intentId))
                return;
            off();
            if (this.isKindOwned(kindId)) {
