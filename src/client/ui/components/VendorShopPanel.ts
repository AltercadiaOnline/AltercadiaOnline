...
import { listInventoryNpcBlockedRows, listInventorySellRows, } from '../vendor/inventorySellRows.js';
...
export class VendorShopPanel extends BaseUIComponent {
    ...
    vendor = { vendorId: VENDEDOR_NPC, vendorName: 'Vendedor' };
    ...
    unsubWallet = null;
    unsubInventory = null;
    purchaseGateway = new ActionGatewayButtonController(() => this.buildPurchaseGatewayOptions());
    sellGateway = new ActionGatewayButtonController(() => this.buildSellGatewayOptions());
    constructor() {
        super({
            id: 'vendorShop',
    ...
    open() {
        if (openReactMovablePanel(this, 'vendorShop'))
            return;
        super.open();
    }
    close() {
        if (closeReactMovablePanel(this, 'vendorShop'))
            return;
        super.close();
    }
    focus() {
        if (focusReactMovablePanel(this, 'vendorShop'))
            return;
        super.focus();
    }
    getRootElement() {
    ...
    onOpen() {
        ...
        this.unsubInventory = this.dataStore.subscribe('inventory', (inventory) => {
            ...
            if (this.isOpen() && this.selectedItemId) {
                ...
                if (this.tradeMode === 'inventory')
                    this.render();
            }
        });
    }
    afterRender() {
        this.purchaseGateway.attach(this.query('[data-action="confirm-purchase"]'));
        this.sellGateway.attach(this.query('[data-action="confirm-sell"]'));
    }
    ...
    createTemplate() {
        ...
        const selectedListing = this.tradeMode === 'catalog' && this.selectedItemId
            ? findNpcVendorListing(this.vendor.vendorId, this.selectedItemId)
            : null;
        const selectedInventoryRow = this.tradeMode === 'inventory' && this.selectedItemId
    ...
    updateWalletLabel() {
        const el = this.query('[data-vendor-wallet]');
        if (el)
    ...
    updateTradeTotals() {
        if (!this.selectedItemId)
            return;
        const sellEl = this.query('[data-vendor-sell-total]');
        if (!sellEl)
            return;
        if (this.tradeMode === 'catalog') {
            ...
            const buyQuote = resolveNpcPurchaseQuote(listing, this.tradeQuantity);
            const sellQuote = resolveNpcSellQuote(listing, this.tradeQuantity);
            const buyEl = this.query('[data-vendor-buy-total]');
            if (buyEl && buyQuote)
                buyEl.textContent = formatVoltsShort(buyQuote.totalVolts);
            if (sellQuote)
                sellEl.textContent = formatVoltsShort(sellQuote.totalVolts);
            return;
        }
    ...
    clampTradeQuantity() {
        ...
        if (this.tradeMode === 'inventory' && this.selectedItemId) {
            ...
            if (owned > 0)
                this.tradeQuantity = Math.min(this.tradeQuantity, owned);
        }
    }
    bindEvents() {
    ...
    buildPurchaseGatewayOptions() {
        return {
            getLabelHtml: () => {
                if (!this.selectedItemId || this.tradeMode !== 'catalog')
                    return 'Comprar';
                const listing = findNpcVendorListing(this.vendor.vendorId, this.selectedItemId);
                if (!listing)
                    return 'Comprar';
            ...
            relatedElements: () => {
                const qty = this.query('[data-vendor-qty]');
                return qty ? [qty] : [];
            },
            onClick: () => {
                if (!this.selectedItemId || this.tradeMode !== 'catalog')
