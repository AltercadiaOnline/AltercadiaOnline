...
import { patchLevelProgressionSection, renderLevelProgressionSection, resolveExplorationSpeedBonusFromAgility, } from '../character/levelProgressionSection.js';
import { buildOperativeEventLogLines, renderOperativeEventLog, } from '../character/characterPanelAchievementLog.js';
import { patchEstiloLine, renderEstiloLine, resolveEstiloName, } from '../character/characterPanelEstilo.js';
import { getGlobalPlayerStore } from '../moveset/globalPlayerStore.js';
import { renderSyncSignalBars, resolveMapSyncStatus, } from '../character/characterPanelSyncStatus.js';
...
export class CharactersPanel extends BaseUIComponent {
    ...
    unsubLoadout = null;
    petSnapshot = null;
    syncStatus = resolveMapSyncStatus(null);
    eventLogLines = [];
    resolvedEstiloName = null;
    constructor() {
        super({
            id: 'characters',
    ...
    bindEvents() {
        this.root?.addEventListener('click', (event) => {
            ...
            const skinSlot = target.closest('[data-skin-slot]');
            if (skinSlot?.dataset.skinSlot) {
                this.toggleSkinMenu(skinSlot.dataset.skinSlot);
                return;
            }
            const skinOption = target.closest('[data-skin-option]');
            if (skinOption?.dataset.skinOption && skinOption.dataset.skinSlot) {
                const slot = skinOption.dataset.skinSlot;
    ...
    shouldRefreshPetBlock(before, after) {
        if (before === after)
            return false;
        if (!before || !after)
            return before !== after;
    ...
    refreshPetBlock() {
        const section = this.query('[data-pet-section]');
        if (!section)
            return;
    ...
    paintPetIcon() {
        const pet = this.petSnapshot;
        const canvas = this.query('[data-pet-icon]');
        if (!pet || !canvas)
            return;
    ...
    refreshProfileBlock() {
        this.syncExplorationSpeedFromEquipment();
        const progressionSection = this.query('[data-level-progression-section]');
        if (progressionSection) {
            patchLevelProgressionSection(progressionSection, this.buildLevelProgressionModel());
        }
        const { profile } = this.state;
        const pvp = this.query('[data-pvp-grid]');
        if (pvp)
            pvp.innerHTML = this.renderPvp(profile);
    }
    ...
    refreshPreview() {
        const canvas = this.query('[data-char-preview]');
        if (canvas)
            paintCharacterPanelPreview(canvas, this.state.skinState.skin);
    }
    toggleSkinMenu(slot) {
        this.state = {
            ...this.state,
    ...
    refreshSkinSlots() {
        const container = this.query('.character-wardrobe__slots');
        if (container)
            container.innerHTML = this.renderSkinSlots();
    }
    refreshSkinMenu() {
        const host = this.query('.character-wardrobe');
        const existing = this.query('[data-skin-menu]');
        if (!host)
            return;
    ...
    refreshEquipmentGrid() {
        const grid = this.query('[data-equip-grid]');
        if (grid)
            grid.innerHTML = this.renderEquipmentGrid(this.state.equipmentGrid);
        const meta = this.query('.character-sheet__sprite-meta');
        if (meta) {
            meta.textContent = formatSpriteMetaLine(this.state.equipmentMeta.displayName, this.state.equipmentMeta.level, this.state.equipmentMeta.classId);
        }
    }
    buildLevelProgressionModel() {
    ...
    syncExplorationSpeedFromEquipment() {
    ...
    refreshSyncIndicator() {
        const host = this.query('[data-sync-indicator]');
        if (!host)
            return;
        ...
        const statusEl = this.query('[data-sync-status]');
        if (statusEl)
            statusEl.textContent = label;
        const barsEl = this.query('[data-sync-bars]');
        if (barsEl)
            barsEl.innerHTML = renderSyncSignalBars(signalBars);
    }
    refreshWalletBlock() {
        const wallet = getDataStore().getWallet();
        const vlt = this.query('[data-wallet-vlt]');
        const alt = this.query('[data-wallet-alt]');
        if (vlt)
            vlt.textContent = wallet.voltsFormatted;
        if (alt)
            alt.textContent = wallet.alterFormatted;
    }
    resolveEstiloOnOpen() {
        const dataStore = getDataStore();
        this.resolvedEstiloName = resolveEstiloName(getGlobalPlayerStore().getConfirmedLoadout(), dataStore.getMarcosState());
        if (this.root)
            patchEstiloLine(this.root, this.resolvedEstiloName);
    }
    refreshEventLog() {
        const list = this.query('[data-event-log-list]');
        if (list)
            list.innerHTML = renderOperativeEventLog(this.eventLogLines);
    }
}
