...
export class RefractionBoothPanel extends BaseUIComponent {
    ...
    open() {
        if (openReactMovablePanel(this, 'refractionBooth'))
            return;
        super.open();
    }
    close() {
        if (closeReactMovablePanel(this, 'refractionBooth'))
            return;
        super.close();
    }
    focus() {
        if (focusReactMovablePanel(this, 'refractionBooth'))
            return;
        super.focus();
    }
    getRootElement() {
    ...
    renderIdleTemplate() {
        ...
        const startDisabled = this.quoteLoading || cooldownMs > 0 || !canAfford || this.completing ? ' disabled' : '';
        return `
    ...
    renderPlayingTemplate() {
        ...
        const missClass = this.misses >= missLimit - 3 ? ' refraction-booth__hud-misses--danger' : '';
        return `
    ...
    mountArenaController() {
        const arena = this.query('[data-refraction-arena]');
        if (!arena)
            return;
    ...
    startTimerLoop() {
        const frame = () => {
            if (!this.session || this.phase !== 'playing' || this.completing)
                return;
    ...
    updatePlayingHudStats() {
        if (!this.isOpen() || this.phase !== 'playing')
            return;
        const hud = this.query('.refraction-booth__hud');
        if (!hud)
            return;
        const missLimit = REFRACTION_BOOTH_CONFIG.maxMisses;
        ...
        if (missEl) {
            ...
            missEl.classList.toggle('refraction-booth__hud-misses--danger', this.misses >= missLimit - 3);
        }
        const spans = hud.querySelectorAll('span');
        if (spans[0])
            spans[0].textContent = `Hits: ${this.hits}`;
        if (spans[2])
            spans[2].textContent = `Score: ${calculateRefractionBoothScore(this.hits, this.misses)}`;
    }
    updateTimerHeader() {
        if (!this.session)
            return;
        const title = this.query('.refraction-booth__title');
        if (title) {
            title.textContent = this.formatDuration(Math.max(0, this.session.expiresAtMs - Date.now()));
        }
    }
    async finishSession() {
        if (!this.session || this.completing)
            return;
