...
import { patchSidebarLevelProgression, renderSidebarLevelProgression, } from '../character/levelProgressionSection.js';
...
import { InventoryService, isSyncPending, selectPlayerEquipment, } from '../../services/index.js';
...
export class EquipmentSidebar {
    ...
    bindSetGridInteraction() {
        this.gridEl.addEventListener('dblclick', (event) => {
            if (InventoryService.isInventoryMutationPending())
                return;
            const button = event.target.closest('[data-equip-slot]');
            const slotId = button?.dataset.equipSlot;
            if (!slotId)
                return;
            if (InventoryService.isInventoryMutationPending())
                return;
    ...
    bindSetGridTooltips() {
        const cleanups = [];
        const buttons = this.gridEl.querySelectorAll('[data-equip-slot][data-item-id]');
    ...
    renderLevelProgression(profile) {
...
export function getEquipmentSidebar() {
    return activeSidebar;
}
export function destroyEquipmentSidebar() {
