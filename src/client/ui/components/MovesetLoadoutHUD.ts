...
export class MovesetLoadoutHUD extends BaseUIComponent {
    snapshot = getGlobalPlayerStore().getSnapshot();
    classId = getPlayerEquipmentStore().getSnapshot().classId;
    movesProgression = getDataStore().getMovesProgression();
    characterLevel = getDataStore().getCharacterLevel().level;
    unsubscribeStore = null;
    unsubscribeEquipment = null;
    unsubscribeProgression = null;
    unsubscribeCharacterLevel = null;
    unbindTooltipListeners = null;
    confirmFeedbackActive = false;
    ...
    bindEvents() {
        this.root?.addEventListener('click', (event) => {
            ...
            if (this.confirmFeedbackActive || this.confirmInFlight)
                return;
            const poolCard = target.closest('[data-pool-move]');
            if (poolCard?.dataset.poolMove) {
                getGlobalPlayerStore().toggleActiveMove(poolCard.dataset.poolMove);
                return;
            }
            const activeSlot = target.closest('[data-active-move]');
            if (activeSlot?.dataset.activeMove) {
                getGlobalPlayerStore().removeActiveMove(activeSlot.dataset.activeMove);
            }
        });
    }
    async handleConfirmLoadout() {
        if (this.confirmFeedbackActive || this.confirmInFlight)
            return;
        const button = this.query('[data-action="confirm-loadout"]');
    ...
    showConfirmSuccessFeedback(button) {
        this.confirmFeedbackActive = true;
        button.disabled = true;
        const label = button.querySelector('.loadout-confirm-btn__label');
        const applySuccessVisuals = () => {
            if (label)
                label.textContent = 'LOADOUT CONFIRMADO!';
    ...
    clearConfirmFeedback() {
        ...
        const label = this.query('.loadout-confirm-btn__label');
        if (label)
            label.style.opacity = '';
        this.confirmFeedbackActive = false;
        this.confirmInFlight = false;
    ...
    afterRender() {
        ...
        if (!this.root)
            return;
        const moveElements = this.root.querySelectorAll('[data-move-id]');
        const cleanups = [];
        for (const element of moveElements) {
            const onEnter = (event) => {
                const target = event.target;
                if (target instanceof Element && target.closest('[data-progression-tooltip]'))
                    return;
    ...
    renderMoveProgress(moveId, moveName) {
        ...
        const progressionAttrs = renderProgressionTooltipAttrs(buildMoveMasteryProgressionTooltip(moveId, progression, moveName));
