 ...
 */
export function renderStatusContainer(container, statuses, options = {}) {
...
function bindStatusChipTooltip(el, chip) {
    const onEnter = () => {
...
function buildStatusChipElement(doc, chip) {
    ...
    el.setAttribute('aria-label', `${resolveStatusTooltipAria(chip.id, chip.stacks, chip.turnsRemaining)}`);
...
function resolveStatusTooltipAria(statusId, stacks, turnsRemaining) {
