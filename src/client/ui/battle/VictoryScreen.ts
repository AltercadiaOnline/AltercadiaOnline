...
import { mountLootCasinoSpin } from './LootCasinoSpin.js';
/** Centro da cena de combate quando visível; senão body (overlay fullscreen). */
function resolveVictoryMountRoot(mountRoot) {
    if (mountRoot instanceof HTMLElement)
        return mountRoot;
    const combat = document.querySelector('#scene-combat');
...
export function showVictoryScreen(options) {
    ...
    return new Promise((resolve) => {
        ...
        let phase = showCasino ? 'intro' : 'ready';
        let casino = null;
        let settled = false;
        const finish = () => {
            if (settled)
