...
export class BattleArenaCanvas {
  ...
  async bindCreature(creatureId: string, label?: string): Promise<void> {
    ...
    this.foe.attackSrc = buildCreatureAttackSpriteSrc(creatureId) || primary;
    this.foe.label = label ?? creatureId;
  ...
  private paint(): void {
    ...
    if (this.ally.image) {
      drawSpriteBottom(ctx, this.ally.image, allyX + allyOffset, allyGroundY, ALLY_DRAW_H, allyFlash);
    }
    if (this.foe.image) {
