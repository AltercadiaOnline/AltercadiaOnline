...
export function resolvePlayerHpTooltipFromStores() {
    ...
    const breakdown = resolvePlayerHpBonusBreakdownFromLoadoutInput(buildHudCombatLoadoutInput({
...
export function showPlayerHpTooltip(clientX, clientY) {
    const tip = resolvePlayerHpTooltipFromStores();
    showHintTooltip(tip.title, clientX, clientY, { lines: tip.lines });
}
export function hidePlayerHpTooltip() {
    hideGameTooltip();
}
/** Bind hover na ficha (HTML string) — barra/valor de HP. */
export function bindPlayerHpTooltipHost(host) {
    const onEnter = (event) => {
        const target = event.target;
        if (!(target instanceof Element))
            return;
        if (!target.closest('[data-level-vital="hp"]'))
            return;
    ...
    const onLeave = (event) => {
        ...
        if (related instanceof Node && host.contains(related)) {
            const stillOnHp = related instanceof Element && Boolean(related.closest('[data-level-vital="hp"]'));
            if (stillOnHp)
                return;
        }
        hidePlayerHpTooltip();
