...
export async function beginBattleEnterSceneFade() {
...
export async function completeBattleEnterSceneFadeIfPending() {
    if (!pendingEnterReveal || !isPhaserRenderEngineActive())
        return;
    const overlay = resolveOverlay();
...
export function hasPendingBattleEnterReveal() {
    return pendingEnterReveal;
}
/** Ramo Phaser — espelha `exitWithFade` com callback no pico preto. */
export async function runPhaserBattleExitFade(onMidFade) {
    if (!isPhaserRenderEngineActive()) {
        onMidFade?.();
        return;
    }
...
export function resetBattleSceneTransitionFade() {
    pendingEnterReveal = false;
    const overlay = resolveOverlay();
    if (!overlay)
        return;
