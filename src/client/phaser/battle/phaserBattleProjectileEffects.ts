// @ts-nocheck
import { blendClassTint, PHASER_PROJECTILE_STYLES, } from './phaserBattleProjectileStyles.js';
const VFX_DEPTH = 24;
function waitMs(ms) {
...
function styledColors(style, classId) {
    return {
...
function placeBar(gfx, x1, y1, x2, y2, thickness, color, alpha = 1) {
    const mx = (x1 + x2) / 2;
    const my = (y1 + y2) / 2;
...
function drawOrb(gfx, style, colors) {
...
function drawDiamond(gfx, style, colors) {
...
function drawSlash(gfx, colors, progress) {
...
function drawHex(gfx, colors, scale) {
...
function drawCross(gfx, colors, scale) {
    const s = 10 * scale;
...
async function animateTravel(scene, gfx, style, colors, from, to, redraw, durationMs, options = {}) {
    const trailGfx = [];
    ...
    while (true) {
        ...
        if (options.spin)
            gfx.rotation = t * Math.PI * 2;
        if (options.flicker)
            gfx.alpha = 0.65 + Math.sin(t * 28) * 0.35;
        gfx.clear();
        redraw();
    ...
    for (const trail of trailGfx)
        trail.destroy();
}
export async function playPhaserProjectileEffect(ctx) {
            ...
            case 'travel': {
                const redraw = () => {
                    if (style.shape === 'diamond')
                        drawDiamond(gfx, style, colors);
                    else if (style.shape === 'bolt')
                        drawBolt(gfx, colors);
                    else
                        drawOrb(gfx, style, colors);
                };
                await animateTravel(ctx.scene, gfx, style, colors, ctx.from, ctx.to, redraw, style.durationMs, { spin: style.shape === 'diamond', flicker: style.shape === 'bolt' });
                break;
            }
            case 'arc': {
                const mid = {
