...
import { getCachedWorldAssetImage, preloadWorldAssetImage, } from '../../world/worldAssetImageLoader.js';
...
import { mountPhaserLayoutRoots, queueStructureLayoutPreloads, } from '../layout/phaserLayoutScene.js';
import { getCachedPropAliasImage, listPropAliasKeys, preloadPropAliasImage, propAliasTextureKey, } from '../../world/propAliasImageLoader.js';
...
import { isRegistryAssetRenderable, positionRegistryAssetAtFeet, queueTilesetAtlasPreload, tryCreateRegistrySprite, } from '../assets/phaserAssetRegistry.js';
...
function structureTextureKey(assetKey) {
    return `altercadia-structure-${assetKey}`;
}
function tryRegisterPropAliasTexture(textures, assetKey) {
...
function tryRegisterLegacyStructureTexture(textures, assetKey) {
    ...
    return {
        ready: true,
        trimmed: resolveTrimmedAssetSourceRect(cached.naturalWidth, cached.naturalHeight, STRUCTURE_TRIM),
    };
}
function resolveStructureAssetKey(snapshot) {
    if (snapshot.kind === 'portal') {
        return null;
    }
    return resolveAssetId(snapshot.assetKey);
}
function resolveActorAssetKey(actor) {
    if (actor.kind !== 'npc')
        return null;
...
export class PhaserStructureController {
    ...
    scene = null;
    roots = null;
    hasRenderedStructures = false;
    mount(scene, existingRoots) {
        this.scene = scene;
    ...
    sync(structures, timestampMs, actors = []) {
        const scene = this.scene;
        const ySortContainer = this.roots?.ySortContainer;
        if (!scene || !ySortContainer)
            return;
        void timestampMs;
    ...
    ensureStructureNode(scene, container, snapshot) {
        if (snapshot.kind === 'portal') {
            return;
        }
        ...
        if (useRegistry && registryKey && asset) {
            ...
            if (created?.sprite) {
                ...
                positionRegistryAssetAtFeet(node.assetSprite, asset, feetX, feetY, snapshot.widthPx, snapshot.heightPx);
        ...
        if (legacy.ready && legacy.trimmed) {
            ...
            sprite.setCrop(legacy.trimmed.sx, legacy.trimmed.sy, legacy.trimmed.sw, legacy.trimmed.sh);
            sprite.setPosition(Math.floor(feetX), Math.floor(feetY));
            normalizePhaserAsset(sprite, legacy.trimmed.sw, legacy.trimmed.sh, snapshot.widthPx, snapshot.heightPx, `${snapshot.assetKey}.png`);
        ...
        if (tryRegisterPropAliasTexture(scene.textures, snapshot.assetKey)) {
            ...
            if (aliasImage) {
                normalizePhaserAsset(node.assetSprite, aliasImage.naturalWidth, aliasImage.naturalHeight, snapshot.widthPx, snapshot.heightPx, `${snapshot.assetKey}.png`);
            }
            node.assetSprite.setDepth(resolvePhaserWorldDepth(feetY));
            node.assetSprite.setVisible(true);
            return;
    ...
    ensureActorNode(scene, container, actor, actorKey) {
        if (actor.kind !== 'npc') {
            return;
        }
        ...
        const renderable = Boolean(assetKey && asset && isRegistryAssetRenderable(scene.textures, assetKey));
        ...
        if (renderable && assetKey && asset) {
            ...
            if (created?.sprite) {
                ...
                node.placeholder?.setVisible(false);
                positionRegistryAssetAtFeet(node.assetSprite, asset, actor.feetX, actor.feetY, targetW, targetH);
...
export function queueStructurePreloads(scene) {
