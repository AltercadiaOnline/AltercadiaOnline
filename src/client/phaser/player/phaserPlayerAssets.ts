...
import { resolvePlayerCardinalRotationUrls, resolvePlayerSkinBundleSouthPreviewUrl, } from '../../../shared/character/playerSkinBundle.js';
...
function playerRotationTextureKey(direction) {
 ...
 */
export async function ensurePlayerSheetTexture(textures) {
    ...
    let loadedAny = false;
    const cardinalDirections = ['south', 'east', 'north', 'west'];
    ...
    for (const [direction, frame] of Object.entries(catalog.rotations)) {
        if (!frame?.image || frame.image.naturalWidth <= 0)
            continue;
...
export function resolvePlayerPhaserTextureKey(direction) {
    const rotationKey = playerRotationTextureKey(direction);
    return rotationKey;
}
export function isPlayerRotationTextureKey(key) {
    return key.startsWith(`${PHASER_PLAYER_TEXTURE_KEY}-rot-`);
}
/** URLs candidatas — útil para preload via Phaser.Loader. */
export function resolvePrimaryPlayerSheetUrl() {
    const bundleId = getActivePlayerSkinBundleId();
    return (PlayerSpriteLoader.resolveTopDownSheetUrls(bundleId).find((url) => url.endsWith('.png'))
...
export function resolvePlayerRotationPreloadEntries() {
    const bundleId = getActivePlayerSkinBundleId();
