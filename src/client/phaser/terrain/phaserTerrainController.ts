...
import { getCachedGroundTile, preloadGroundTile, } from '../../world/groundTileImageLoader.js';
import { PHASER_TEXTURE_FILTER_NEAREST } from '../player/phaserPlayerAssets.js';
import { CITY_01_MAP_CONFIG } from '../layout/MapConfig.js';
import { getTerrainLayoutStyle, resolveTerrainLayoutKind, } from '../layout/terrainLayoutPalette.js';
import { ensureGroundPlaceholderTexture, ensurePlaceholderTexture, } from '../layout/phaserDrawPlaceholder.js';
import { PlaceholderType } from '../../world/placeholderRenderer.js';
import { mountPhaserLayoutRoots, queueTerrainLayoutPreloads, } from '../layout/phaserLayoutScene.js';
...
import { isRegistryAssetRenderable, positionRegistryTileAtCorner, queueTilesetAtlasPreload, tryCreateRegistrySprite, } from '../assets/phaserAssetRegistry.js';
function groundTileTextureKey(tileId) {
    return `altercadia-ground-${tileId}`;
}
function tileInstanceKey(tile) {
    return `${tile.worldX}:${tile.worldY}:${tile.size}`;
}
function resolveTerrainAssetKey(tile) {
    if (tile.groundTileId) {
        return resolveAssetId(tile.groundTileId);
    }
    return resolveAssetId(tile.placeholderType);
}
function tryRegisterLegacyGroundTexture(textures, tileId) {
...
export class PhaserTerrainController {
    ...
    ensureAndUpdate(scene, container, tile, instanceKey) {
        ...
        if (registryReady && assetKey && asset) {
            ...
            positionRegistryTileAtCorner(node.assetSprite, asset, tile.worldX, tile.worldY, tile.size);
        ...
        if (tile.groundTileId) {
            ...
            if (textureKey) {
                ...
                normalizePhaserAsset(node.assetSprite, sourceW, sourceH, tile.size, tile.size, `${tile.groundTileId}.png`);
                node.assetSprite.setVisible(true);
                return;
            }
        }
        ...
        if (!node.layoutRect) {
            const layoutRect = scene.add.rectangle(tile.worldX + tile.size / 2, tile.worldY + tile.size / 2, tile.size, tile.size, style.fill);
...
export function queueGroundTilePreloads(scene) {
