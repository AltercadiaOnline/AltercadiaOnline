// @ts-nocheck
import { tiledObjectTextureKey, tiledTilesetTextureKey, } from '../../../config/tiledMapManifest.js';
import { resolveTiledPublicAssetUrl } from './tiledAssetPaths.js';
import { queueProcessedTilesetAtlas, } from './processedTilesetPreload.js';
import { tiledSharedTilesetTextureKey, tiledTilesetLookupKey, } from './tiledTextureKeys.js';
...
export class TiledAssetManager {
    ...
    tilesetTextureKeyByLookup = new Map();
    /** url pública → textureKey canônica (dedupe de preload). */
    sharedKeyByImageUrl = new Map();
    queueMapAssets(scene, descriptor) {
        ...
        for (const tileset of descriptor.tilesets) {
            ...
            const processedAtlasKey = queueProcessedTilesetAtlas(scene, publicUrl, queuedTextureKeys);
        ...
        for (const imagePath of descriptor.objectImages) {
            this.queueImage(scene, tiledObjectTextureKey(descriptor.cacheKey, imagePath), resolveTiledPublicAssetUrl(descriptor.jsonUrl, imagePath), queuedTextureKeys);
        }
     ...
     */
    resolveTilesetTextureKey(mapCacheKey, tilesetName) {
     ...
     */
    ensureEnrichedTilemapInCache(scene, descriptor) {
        if (!descriptor.phaserMapData) {
            throw new Error(`[TiledAssetManager] Artefato Phaser ausente para "${descriptor.mapId}" — rode npm run mirror:map-mund e npm run build.`);
        }
    ...
    registerTilemapData(scene, descriptor) {
        this.ensureEnrichedTilemapInCache(scene, descriptor);
    }
    tilesetTextureKey(mapCacheKey, tilesetName) {
        return tiledTilesetTextureKey(mapCacheKey, tilesetName);
    }
    objectTextureKey(mapCacheKey, imagePath) {
        return tiledObjectTextureKey(mapCacheKey, imagePath);
    }
    resolvePublicUrl(mapJsonUrl, tiledImagePath) {
        return resolveTiledPublicAssetUrl(mapJsonUrl, tiledImagePath);
    }
    classifyAssetFolder(publicUrl) {
        if (publicUrl.includes(TERRAIN_PATH_PREFIX))
            return 'terrain';
        if (publicUrl.includes(STRUCTURES_PATH_PREFIX))
            return 'structures';
        if (publicUrl.includes(PROPS_PATH_PREFIX))
            return 'props';
        return 'other';
    }
    queueTilesetTexture(scene, publicUrl, textureKey, queued) {
        if (queued.has(textureKey))
            return;
        queued.add(textureKey);
        scene.load.image(textureKey, publicUrl);
    }
    resolveSharedTextureKey(mapCacheKey, publicUrl) {
    ...
    queueImage(scene, textureKey, url, queued) {
        if (queued.has(textureKey))
            return;
        queued.add(textureKey);
...
export function getTiledAssetManager() {
    if (!activeManager)
        activeManager = new TiledAssetManager();
    return activeManager;
}
/** Limpa registro de aliases — útil ao trocar de instância de mapa. */
export function resetTiledAssetManager() {
    activeManager = null;
}
