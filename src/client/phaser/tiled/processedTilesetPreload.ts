...
export const ROAD2_SOURCE_PUBLIC_URL = '/assets/terrain/tiles/craftpix-net-574220-free-path-and-road-top-down-pixel-tileset/PNG_Tiled/Road2.png';
 ...
 */
export function processedTilesetAtlasKeyFromSourceUrl(sourcePublicUrl) {
...
export function processedTilesetAtlasKey(tilesetName) {
    return `processed:tileset:${tilesetName}`;
}
export function resolveProcessedTilesetForPublicUrl(publicUrl) {
 ...
 */
export function queueProcessedTilesetAtlas(scene, sourcePublicUrl, queued) {
    ...
    if (atlasKey === ROAD2_ATLAS_TEXTURE_KEY || atlasKey === ZONE1_TOPDOWN_CREATURES_ATLAS_KEY) {
        console.error(`[processedTilesetPreload] ${atlasKey} ausente no cache — PreloaderScene deve completar antes do mapa.`);
        return atlasKey;
    }
...
export function queueRoad2ProcessedAtlas(scene, queued) {
