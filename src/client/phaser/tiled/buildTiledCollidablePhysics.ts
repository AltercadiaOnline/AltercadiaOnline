...
import { applyTiledArcadeColliderBody, resolveTiledColliderFeetAnchor, } from '../../../shared/world/tiledObjectCollisionHitbox.js';
...
function ensureColliderPlaceholderTexture(scene) {
 ...
 */
export function buildTiledCollidablePhysicsBodies(scene, mapId, rawMap) {
    const physics = scene.physics?.add;
    if (!physics?.staticGroup)
        return null;
    ...
    forEachTiledCollidableObject(rawMap, ({ object, kind }) => {
        ...
        const sprite = group.create(anchor.feetX, anchor.feetY, textureKey);
        ...
        if (!body)
            return;
        applyTiledArcadeColliderBody(body, anchor.width, anchor.height, kind === 'npc' ? 'npc' : 'prop');
        created += 1;
    });
    if (created === 0) {
        group.destroy(true);
    ...
    console.info(`[MapLoader] Colisão Arcade — ${created} corpo(s) estático(s) para mapa "${mapId}".`);
    return group;
}
