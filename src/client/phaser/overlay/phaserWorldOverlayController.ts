...
function drawNavigationDestinationMarker(graphics, worldX, worldY) {
...
export class PhaserWorldOverlayController {
    ...
    redrawOverlay(frame, minimap, options) {
        ...
        if (frame.navigationDestination) {
            drawNavigationDestinationMarker(layer, frame.navigationDestination.worldX, frame.navigationDestination.worldY);
        }
        if (!minimap)
            return;
        ...
        if (minimap.destination) {
            ...
            layer.strokeRect(minimap.destination.tileX * TILE_SIZE + 4, minimap.destination.tileY * TILE_SIZE + 4, TILE_SIZE - 8, TILE_SIZE - 8);
            layer.fillStyle(0xf7d774, 0.35);
            layer.fillCircle(destX, destY, 3);
        }
    }
    async syncDebugOverlay(frame) {
