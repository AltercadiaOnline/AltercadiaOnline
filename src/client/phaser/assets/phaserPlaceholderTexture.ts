...
function blendHue(base, target, factor) {
...
function abbreviateLabel(seed, max = 3) {
...
function drawSilhouette(ctx, seed, kind, width, height) {
...
function drawTilePattern(ctx, seed, width, height) {
...
function drawPropPattern(ctx, seed, width, height, label) {
...
function drawGenericPattern(ctx, seed, width, height, label) {
    const hue = hashSeed(seed) % 360;
    ctx.clearRect(0, 0, width, height);
...
function drawPlaceholder(ctx, options) {
...
export function inferPlaceholderKind(textureKey, assetUrl) {
    const haystack = `${textureKey} ${assetUrl ?? ''}`.toLowerCase();
    if (haystack.includes('player') || haystack.includes('altercadia-player'))
        return 'player';
    if (haystack.includes('creature') || haystack.includes('/creatures/'))
        return 'creature';
    if (haystack.includes('npc') || haystack.includes('/npcs/'))
        return 'npc';
    if (haystack.includes('tileset') || haystack.includes('/terrain/'))
        return 'tile';
    if (haystack.includes('/props/')
...
export function defaultPlaceholderSize(kind) {
    switch (kind) {
        case 'player':
            return {
 ...
 */
export function ensurePhaserPlaceholderTexture(textures, options) {
...
export function ensureTextureOrPlaceholder(textures, textureKey, seed, kind, width, height, label) {
    if (textures.exists(textureKey))
        return textureKey;
...
export function ensurePlaceholdersForFailedKeys(textures, failedKeys, resolveUrl) {
    let created = 0;
    for (const textureKey of failedKeys) {
        if (SKIP_PLACEHOLDER_TEXTURE_KEYS.has(textureKey))
            continue;
        if (textures.exists(textureKey))
            continue;
...
function drawMissingGidPattern(ctx, width, height) {
    ctx.clearRect(0, 0, width, height);
...
export function ensureTiledMissingGidTexture(textures, width = DESIGN_CONFIG.TILE.SIZE, height = DESIGN_CONFIG.TILE.SIZE) {
