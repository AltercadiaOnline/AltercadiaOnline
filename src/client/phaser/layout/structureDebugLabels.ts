...
export function resolveStructureDebugLabel(snapshot) {
    ...
    const fromMap = CITY_01_MAP_CONFIG.structureMarkers.find((marker) => marker.id === snapshot.assetKey)?.label;
    if (fromMap) {
        return fromMap;
    }
...
export function resolveActorDebugLabel(actor) {
    if (actor.kind === 'npc') {
...
export function resolveStructureDebugColors(snapshot) {
    if (snapshot.kind === 'portal') {
...
export function resolveActorDebugColors(actor) {
    if (actor.kind === 'npc') {
