...
function strokeRect(ctx, x, y, w, h, color, lineWidth = 2) {
...
function drawGroundTile(ctx, w, h, kind) {
    ctx.clearRect(0, 0, w, h);
    let fill = CYBERPUNK.asphalt;
    let stroke = CYBERPUNK.cyanDim;
    let accent = CYBERPUNK.cyan;
    switch (kind) {
...
function drawVendingMachine(ctx, w, h) {
    ctx.clearRect(0, 0, w, h);
    fillRect(ctx, 0, 0, w, h, CYBERPUNK.slateDark);
    strokeRect(ctx, 0, 0, w, h, CYBERPUNK.cyan, 2);
...
function drawStreetLight(ctx, w, h) {
    ctx.clearRect(0, 0, w, h);
...
function createPlaceholderCanvas(type, w, h, groundKind) {
...
function textureKey(type, w, h, groundKind) {
    const gk = groundKind ?? 'none';
    return `${TEX_PREFIX}:${type}:${gk}:${w}x${h}`;
}
export function ensurePlaceholderTexture(textures, type, widthPx, heightPx, groundKind) {
...
export function resolvePlaceholderDrawType(assetKey, placeholderType) {
    const key = assetKey.toLowerCase();
    if (placeholderType === 'ARENA_STEP' || placeholderType === 'TOWER_STEP')
        return 'stairs';
    if (placeholderType === 'ARENA_FLOOR' || placeholderType === 'SPECTATOR_RING')
        return 'arena';
    if (placeholderType === 'RANKING_MONITOR')
        return 'monitor';
    if (placeholderType === 'NPC_SPOT')
        return 'npc';
    if (placeholderType === 'BUILDING' || placeholderType === 'TOWER_BUILDING')
        return 'building';
    if (placeholderType === 'INTERACTIVE_OBJ')
        return 'vending_machine';
    if (placeholderType === 'REFRACTION_BOOTH')
        return 'vending_machine';
    if (TREE_RE.test(key))
        return 'tree';
    if (LIGHT_RE.test(key))
        return 'street_light';
    if (VENDING_RE.test(key))
        return 'vending_machine';
    if (MONITOR_RE.test(key))
        return 'monitor';
    if (BUILDING_RE.test(key))
        return 'building';
    if (NPC_RE.test(key))
        return 'npc';
    return 'prop';
}
export function ensureDrawnPlaceholderSprite(scene, container, existing, options) {
    const type = resolvePlaceholderDrawType(options.assetKey, options.placeholderType);
    const texKey = ensurePlaceholderTexture(scene.textures, type, options.widthPx, options.heightPx);
    if (!texKey)
        return null;
...
export function drawPlaceholder(scene, container, type, options, existing) {
    return ensureDrawnPlaceholderSprite(scene, container, existing ?? null, {
        ...options,
        assetKey: options.assetKey || type,
    });
}
export function ensureGroundPlaceholderTexture(textures, kind, tileSize) {
