 ...
 */
export function resolveDefaultMarketplaceValorBase(item) {
...
export function applyMarketplaceCatalogDefaults(item) {
...
export function isMarketplaceBrowseListableItem(item) {
    if (item.category === ItemCategory.Currency)
        return false;
    if (item.isTradable === false)
        return false;
