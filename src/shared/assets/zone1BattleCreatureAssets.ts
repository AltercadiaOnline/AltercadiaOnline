...
export function resolveZone1BattleSpriteUrl(creatureId) {
    const file = ZONE1_BATTLE_FILES[creatureId];
    if (!file)
        return null;
    return `${ZONE1_BATTLE_SPRITE_BASE}/${file}`;
}
export function hasZone1BattleSprite(creatureId) {
