...
function isMapPlayerSpawnToken(token) {
    if (PLAYER_SPAWN_IDENTIFIERS.has(token))
        return true;
    if (token === 'spawn' || token === 'player')
        return true;
    // Beco / zonas de hunt — ex.: spawn_zona_beco_1
    if (token.startsWith('spawn_zona_') || token.startsWith('player_spawn_'))
        return true;
    return false;
}
/** Objeto Tiled que representa o spawn do jogador. */
export function isTiledPlayerSpawnObject(object) {
    const name = normalizeToken(object.name);
    if (name && isMapPlayerSpawnToken(name))
        return true;
    const type = normalizeToken(object.type);
    if (type && isMapPlayerSpawnToken(type))
        return true;
    ...
    if (typeof spawnKind === 'string') {
        ...
        if (normalized === 'player' || normalized === 'player_spawn')
            return true;
    }
    return false;
}
 ...
 */
export function resolveTiledPlayerSpawnFromLayer(objects) {
...
export function resolveTiledSpawnFeetPosition(object) {
...
export function resolveTiledPlayerSpawn(object) {
    if (!isTiledPlayerSpawnObject(object))
        return null;
