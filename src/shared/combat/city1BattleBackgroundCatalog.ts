...
export const CITY1_BATTLE_BACKGROUNDS = [
    {
        id: 'city1-var-a',
...
export type BattleBackgroundVariant = (typeof CITY1_BATTLE_BACKGROUNDS)[number];
 ...
 */
export function resolveCity1BattleBackground(battleCount) {
    const safeCount = Number.isFinite(battleCount) && battleCount > 0 ? Math.floor(battleCount) : 0;
    const index = Math.floor(safeCount / BATTLE_BACKGROUND_ROTATION_EVERY) % CITY1_BATTLE_BACKGROUNDS.length;
