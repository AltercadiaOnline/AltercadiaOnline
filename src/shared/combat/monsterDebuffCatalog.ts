...
export const MONSTER_DEBUFF_STATUS_IDS = new Set([
...
export function zoneMonsterDebuffSlots(zoneId) {
    return ZONE_DEBUFF_SLOTS[zoneId] ?? 1;
}
export function zoneRecommendedPlayerLevelMax(zoneId) {
 ...
 */
export const MONSTER_DEBUFF_PROFILES = {
...
export const ZONE1_BASIC_ATTACK_SKILL_IDS = {
    rat: 'rat_bite',
    spider: 'spider_bite',
    bat: 'bat_strike',
    crow: 'crow_slash',
...
export function resolveActiveMonsterDebuffSkillIds(creatureId) {
 ...
 */
export function resolveMonsterCombatSkillIds(creatureId) {
...
export function isMonsterDebuffSkillId(skillId) {
    for (const profile of Object.values(MONSTER_DEBUFF_PROFILES)) {
        if (profile.candidates.some((c) => c.skillId === skillId))
            return true;
    }
    return false;
}
export function resolveMonsterDebuffLoreLabel(skillId) {
    for (const profile of Object.values(MONSTER_DEBUFF_PROFILES)) {
        const hit = profile.candidates.find((c) => c.skillId === skillId);
        if (hit)
            return hit.loreLabel;
    }
    return null;
}
