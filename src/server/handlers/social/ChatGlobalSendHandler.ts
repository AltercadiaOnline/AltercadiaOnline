...
import { broadcastChatGlobalPayload, resolveChatGlobalDisplayName, } from '../../chat/chatGlobalBroadcast.js';
 ...
 */
export class ChatGlobalSendHandler extends BaseIntentHandler {
    actionType = 'CHAT_GLOBAL_SEND';
...
let handler = null;
export function getChatGlobalSendHandler() {
    if (!handler)
        handler = new ChatGlobalSendHandler();
    return handler;
}
