...
function resolveWorldVitals(profile) {
...
function mapHealFailureReason(reason) {
...
export class HealAtNpcTransactionHandler extends BaseTransactionHandler {
    actionType = 'HEAL_AT_NPC';
    lastVoltsDebited = 0;
    pendingHeal = null;
    validate(action, profile) {
        ...
        const level = getAuthoritativeProgression(action.playerId, action.characterId).characterProfile.level;
    ...
    async runTransaction(action, profile) {
...
let healHandler = null;
export function getHealAtNpcTransactionHandler() {
    if (!healHandler)
        healHandler = new HealAtNpcTransactionHandler();
    return healHandler;
}
export function resetHealAtNpcTransactionHandler() {
    healHandler = null;
}
